...
if os.path.exists(CONFIG_FILE):
    with open(CONFIG_FILE, "r") as f: config_data.update(json.load(f))
else:
    print("⚠️ Config not found. Entering First-Time Setup...")
    config_data["TELEGRAM_TOKEN"] = input("Enter your Telegram Bot Token: ").strip()
    try: config_data["ADMIN_ID"] = int(input("Enter your Telegram Admin ID: ").strip())
    except ValueError:
        print("\n❌ INVALID FORMAT: User ID must be a number.")
        sys.exit(1)
    save_config()
TELEGRAM_TOKEN, ADMIN_ID = config_data["TELEGRAM_TOKEN"], config_data["ADMIN_ID"]
ALLOWED_ADMINS = [1232325263]
...
try:
    startup_max_spins = int(input("Enter default Maximum Spins per number (e.g. 50): ").strip())
    if startup_max_spins < 1: startup_max_spins = 50
except ValueError: startup_max_spins = 50
